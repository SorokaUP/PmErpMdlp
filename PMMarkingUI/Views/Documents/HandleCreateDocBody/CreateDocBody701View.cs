...
{
    public partial class CreateDocBody701View : UserControl
    {
        DataTable dtData;
        public CreateDocBody701View()
        {
            InitializeComponent();
            this.Text = "Создание документа 701";
            this.Tag = false;
        ...
        private void tsmiActionDoc_Create_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Сформировать документ?", "ВНИМАНИЕ", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                ...
                lst.Add($"	<accept action_id=\"701\">");
                lst.Add($"      <subject_id>{GetStringFromObject(subject_id.Properties.Value)}</subject_id>");
                lst.Add($"      <counterparty_id>{GetStringFromObject(counterparty_id.Properties.Value)}</counterparty_id>");
