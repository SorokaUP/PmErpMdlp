...
namespace RestApi.Additional.old
{
    public class Ahunter
    {
        ...
        /// </summary>
        public static void GetFiasAhunter(int lid = 0)
        {
            int PackLim = 20;
            int cntErrors = 0;
            ...
            try
            {
                ...
                // $" ({DT_AddressesFias.Rows.Count} записей / {cntPackCnt} пакетов)";
        ...
        private static int ArrayToApi(List<ApiAddressRole> adrs)
        {
            ...
            {
                sb.Append("<record id=\"" + (id + 1).ToString() + "\">");
                sb.Append("<item type=\"address\" role=\"TW_ID:" + adrs[id].TW_ID.ToString() + ";ID:" + adrs[id].ID.ToString() + "\">");
